...
fn main() {
    ...
    println!("a: {}", solve_a(&a_input, 2000000).unwrap());
    println!("b: {}", solve_b(&a_input, 4000000).unwrap());
}
...
mod test_main {
    ...
    fn a_test() {
        assert_eq!(
            solve_a(&DataLoader::from_file("data/test_input.txt"), 10).unwrap(),
    ...
    fn b_test() {
        assert_eq!(
            solve_b(&DataLoader::from_file("data/test_input.txt"), 20).unwrap(),
